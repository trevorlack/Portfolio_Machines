...
dstart = datetime.date(2017,5,1)
dend = datetime.date(2017,5,31)
...
if c_link['CUSIP2_9'].count() > 0:
    ...
    hyhg_index_raw.to_csv('test1.csv')
...
best_ticker = hyhg_index_raw[['Volume', 'ID_CUSIP', 'Ticker']].groupby(['ID_CUSIP', 'Ticker']).sum()
best_ticker = best_ticker.reset_index()
'''idxmax function used to return the cusip with the highest volume per issue'''
